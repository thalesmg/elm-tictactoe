...
init : ( Model, Cmd msg )
init =
    ( { arr = emptyArray, turn = Human }, Cmd.none )
...
mkTable m =
    let
        element i j =
            td [ onClick (Play i j) ] [ renderCell (get ( i, j ) m) ]
...
play i j msg model =
    let
        ...
        element =
            joinMaybe <| get ( i, j ) array

        gameOver =
            playerOutcome array currturn |> isJust
    in
        if gameOver then
            ( model, Cmd.none )
        else
            case element of
                Nothing ->
                    computerPlay { model | arr = makeMove currturn ( i, j ) array, turn = otherPlayer currturn }
...
transpose ll =
    case ll of
        (x :: xs) :: xxs ->
            -- We have at least one non-empty list at head
            let
                mheads =
                    xxs
                        |> List.map List.head
...
winner b =
    let
        ts =
            triples b
    in
        if List.member (List.repeat 3 (Just Human)) ts then
            Just Human
        else if List.member (List.repeat 3 (Just Computer)) ts then
            Just Computer
        else
            Nothing
...
playerOutcome b player =
    ...
    in
        case w of
            ...
            Nothing ->
                if List.all (not << isNothing) flatBoard then
                    Just Draw
                else
                    Nothing
...
filterJustTuple lma =
    case lma of
        ...
        ( mx, y ) :: mxsys ->
            case mx of
                Just x ->
                    ( x, y ) :: filterJustTuple mxsys

                Nothing ->
                    filterJustTuple mxsys
...
minimax2 player board =
    let
            ...
            case playerOutcome bb pp of
                ...
                Nothing ->
                    let
                        ...
                        bs =
                            List.map (\pos -> makeMove other pos bb) moves

                        outs =
                            List.map (\b -> helper ( other, (dd + 1), b )) bs

                        ranks =
                            outs |> List.map (\( o, d ) -> ( ( o, d ), rank2 o d )) |> List.sortBy Tuple.second |> List.reverse
                    in
        ...
        gameOver =
            playerOutcome board player |> isJust

        allBoards =
            availableMoves board |> List.map (\pos -> makeMove player pos board)

        tagged =
            allBoards |> List.map (\b -> ( b, helper ( player, 0, b ) |> (\( o, d ) -> rank2 o d) )) |> List.sortBy Tuple.second |> List.reverse
    in
        if gameOver then
            board
        else
